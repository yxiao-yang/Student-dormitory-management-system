...
def Login():
    def Signin():
        ...
        ## 学号密码是否正确
        cur.execute("SELECT COUNT(*) from supervisor where Uno =  '%s'" % str(stu_Un))
        flag1 = cur.fetchone()[0]
        print(flag1)
        if flag1 != 0: ### 学号存在
            cur.execute("SELECT Upassword from supervisor where Uno =  '%s'" % str(stu_Un))
            ...
            messagebox.showerror('警告',message='用户名查找失败，请重新输入')
    ...
    label = Label(root, text = "（管理员端）", font = ("微软雅黑 -20"))
    label.place(x = 250, y = 150, height = 40, width = 100)
    label = Label(root, text = "用户名：", font = ("微软雅黑 -20"))
